...
const Comment = ({ id, description, date, answers, parentId, level, user, postId }) => {
	...
	const [numberAnswers, setNumberAnswers] = useState(answers);
	...
	const handleSubmit = event => {
		...
		if (form.checkValidity()) {
			dispatch(commentActions.commentAnswer(comment, postId, id, page, comments => {
					...
					{numberAnswers > 0 &&
						<div role="button" className="color-text" onClick={() => {
							if (seeAnswers) {
								...
								dispatch(commentActions.findAnswersComment(id, page, comments => setCommentAnswers(comments)))
							}
				...
				<div>
					...
					{commentAnswers.existMoreItems &&
						<button type="button" className="btn btn-secondary mb-4" onClick={() => {
							dispatch(commentActions.nextFindAnswersResultPage(id, page, results => {
